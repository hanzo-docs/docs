...
export default createGui({
  fonts: {
    body: createFont({
      family: 'Zen, ui-sans-serif, system-ui, sans-serif',
      // ...
    }),
    body_cn: createFont({
      family: 'Zen-CN',
...
You'll then need to load the `Zen` and `Zen-CN` fonts for the platforms you support. Once you do, you'll get fully typed support for changing any `body` font to `cn` in a component:
