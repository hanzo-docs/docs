...
- **ML Backend**: upstream [candle](https://github.com/huggingface/candle) from crates.io (`candle-core`, `candle-nn`, `candle-transformers` 0.9)
...
Repo: `hanzoai/edge`. Tensor operations come from upstream
[candle](https://github.com/huggingface/candle) 0.9, resolved from crates.io —
Edge does not depend on `hanzo-ml`.
