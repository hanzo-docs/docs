...
description: "Hanzo O11y is a full-stack observability platform for logs, metrics and traces — OpenTelemetry in, ClickHouse underneath, one query surface instead of three products. Go backend, React frontend. Live at o11y.hanzo.ai."
---

## Overview

Hanzo O11y is a **full-stack observability platform** for logs, metrics, and traces. Go 1.25 backend (Gin/gorilla-mux) with a React 18 + Vite frontend. Uses **ClickHouse** as the telemetry datastore and ingests over **OpenTelemetry**, so any OTLP exporter points at it without a vendor agent. Logs, metrics and traces land in one store and one query surface rather than three products. Live at `o11y.hanzo.ai`.
