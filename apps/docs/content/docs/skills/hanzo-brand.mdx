...
Zen is the Hanzo family and ships as [`@hanzo/font`](https://www.npmjs.com/package/@hanzo/font).
Mount it once at the top of your root stylesheet with `@import '@hanzo/font/css'`,
which declares the faces and publishes `--font-zen-sans` / `--font-zen-mono`; bind
those to whatever `--font-sans` / `--font-mono` pair your token sheet already reads.
