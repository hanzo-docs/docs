...
Hanzo LLM Gateway is a **unified proxy** that routes requests to 100+ LLM providers through a single OpenAI-compatible API. Point any OpenAI SDK at it, change the model string, and the gateway handles routing, fallback chains, retries across providers, and per-request cost attribution. Powers `api.hanzo.ai/v1`.
...
`github.com/hanzoai/llm` — Hanzo Router. Python.
