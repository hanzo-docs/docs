...
Hanzo Chat (`@hanzochat/chat`) is a **full-featured AI chat application** -- a web UI for conversing with AI models, managing conversations, running agents, and integrating with RAG pipelines and MCP tools. It connects to the Hanzo LLM Gateway (`api.hanzo.ai/v1`) as its backend for model access.
