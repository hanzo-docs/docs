...
Hanzo vLLM depends on `guoqingbao/candle` (a fork of `huggingface/candle`) for
tensor operations, neural network layers, and GPU backends — pinned in its
`Cargo.toml`. It does **not** use `hanzo-ml`. The two are separate layers,
built on separate tensor stacks:

- **hanzoai/ml** -- the Hanzo ML framework: tensors, layers, GPU kernels
- **hanzoai/vllm** -- this inference server: OpenAI-compatible API, continuous
  batching, scheduling, PagedAttention

## Related Skills

- `hanzo/hanzo-ml.md` - the Hanzo Rust ML framework (a separate tensor stack)
