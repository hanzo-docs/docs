...
- 6 embedding providers: Mock, OpenAI, ONNX, Transformers.js, llama.cpp, and Candle (present in code but unusable — see the provider table)
...
| Candle | `candle` | Yes | Metal (macOS) | **Unavailable** — shells out to a `candle-embeddings` CLI that is not published on crates.io |
...
- **Candle not found**: there is no fix — the provider shells out to a
  `candle-embeddings` binary that has never been published to crates.io, so
  `cargo install candle-embeddings` cannot succeed. Use `onnx` (the real local
  path, via the `onnxruntime-node` dependency) or `openai`.
