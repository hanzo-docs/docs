...
import { docs } from '@hanzo/docs-mdx:collections/my-workspace/server';
```
...
```ts title="lib/source.ts"
import { loader, multiple } from '@hanzo/docs/source';
...
- when a deployment is triggered on $A$:
  ...
  - each workspace generates its own collection entries, e.g. `@hanzo/docs-mdx:collections/{repo}/server`.
