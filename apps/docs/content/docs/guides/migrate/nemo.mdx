...
| Relay middleware: redaction, policy, allow or deny before execution | The agent's `tools` list — names resolved from `GET /v1/tools` at run time, and omitting one denies it; `zen-guard` on input or output where a content decision is needed |
| `nemo-relay run -- claude` observing a local coding agent | `POST /v1/agents/coding` runs the coding agent on this side of the API, and `GET /v1/agents/sessions/stream` is the live event feed |
...
| NeMo | Hanzo |
|---|---|
| `config.yml` naming a `content_safety` model and the rails that call it | `zen-guard`, called at `POST /v1/chat/completions` on the input, the output or both. The classification is the response |
...
| Jailbreak detection heuristics | `zen-guard` reports the jailbreak category with the rest |
| Rail execution logged per turn | The same trace as the call: each check is one more observation on it |
...
rail did, with the decision in a flow file instead of in the caller.
...
interprets a flow file. Content decisions are a `zen-guard` call whose answer
your code acts on; topical and conversational control is the agent's
`instructions`; what it may do is its `tools` list. A Colang configuration doing
substantive dialog control is rewritten as those three, not ported.
