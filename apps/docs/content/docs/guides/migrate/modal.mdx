---
title: Modal
description: Modal deploys Python functions onto GPUs and calls them from a client. Here that is /v1/function (11 operations) for the function, /v1/sandbox (19) for a sandbox and /v1/compute (34) for the accelerators underneath — with the tenant coming from your key rather than a workspace.
---

Modal runs a Python function on a GPU and hands you a handle to call it.
`/v1/function` (11 operations) is the same unit: publish a record, invoke it by
...
curl -sS -X POST https://api.hanzo.ai/v1/function \
...
curl -sS -X POST https://api.hanzo.ai/v1/function/render/invoke \
  -H "Authorization: Bearer $HANZO_API_KEY" \
  -H 'Content-Type: application/json' \
  -d '{"input":"abc"}'
```
...
`GET /v1/function/{name}/logs`. That is `modal deploy` plus `.remote()` in two
...
|---|---|---|
| `/v1/function` | Publish once, then call it at `POST /v1/function/{name}/invoke` | 11 |
| `/v1/sandbox` | Lease a container at `POST /v1/sandbox/lease`, then exec, read, write | 19 |
| `/v1/compute` | The GPUs a `fleet` function runs on, listed at `GET /v1/compute/gpus` | 34 |
...
| `@app.function(...)` plus `modal deploy` | `POST /v1/function` — one record carrying runtime, code, limits and secret names |
| `.remote(x)` | `POST /v1/function/{name}/invoke`, `input` arriving on the program's stdin |
| Web endpoint and its `*.modal.run` URL | `endpoint` on the record; every function's is listed at `GET /v1/function/triggers` |
| `gpu="H100"` | `target: "fleet"` — the org's own GPU fleet, inventoried at `GET /v1/compute/gpus` |
...
| `modal app list` | `GET /v1/function/deployments` — a function has one record, and that record IS its live deployment |
| `modal app logs` | `GET /v1/function/{name}/logs` — the most recent run's output, and nothing older |
| The function's dashboard page | `GET /v1/function/{name}` — definition, 7-day rollup, trigger, last twenty invocations, secret names, in one round-trip |
| Call history | `GET /v1/function/{name}/invocations` — real recorded rows, newest first, never a projection |
| The usage graphs | `GET /v1/function/metrics` — counted invocations per bucket, with the cost column left null rather than guessed |
...
| Modal's GPU classes and pricing page | `GET /v1/compute/compute/sizes` for what can be launched, `GET /v1/compute/fleet` for what you hold |
| Adding capacity | `POST /v1/compute/machines`, or your own host dialing in with `hanzo link` |
| The GPU work queue | `GET /v1/compute/fleet/jobs` — per-node lanes plus a shared any-GPU lane, cancelled at `POST /v1/compute/fleet/jobs/{id}/cancel` |
...
```bash
curl -sS https://api.hanzo.ai/v1/function/render \
  -H "Authorization: Bearer $HANZO_API_KEY"
```
...
iterable and hands back a `FunctionCall` you poll. `POST /v1/function/{name}/invoke`
is synchronous and answers when the run is over. Fan-out is a queue you drive:
`/v1/mq` (15) for the work, one invoke per item, and `GET /v1/compute/fleet/jobs`
...
`GET /v1/compute/gpus`, added by `POST /v1/compute/machines` or by a host running
...
`min_containers` there and keep `/v1/function` for work that begins on a
request.
