---
title: Twilio
description: Twilio holds phone numbers and moves messages over them. Three capabilities answer it — /v1/tel for the carrier, /v1/notify for transactional delivery, /v1/channel for chat.
---
...
operations), `/v1/notify` (4), `/v1/channel` (7).
...
| `/v1/channel` | Chat on Discord, Slack, Teams and Telegram | 7 |
...
| Conversations on chat transports | `POST /v1/channel/{channel}/send` — `discord`, `slack`, `teams`, `telegram` |
...
**No WhatsApp and no RCS.** `/v1/channel` covers Discord, Slack, Teams and
