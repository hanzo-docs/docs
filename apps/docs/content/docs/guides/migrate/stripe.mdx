...
curl -sS -X POST https://api.hanzo.ai/v1/billing/topup \
  -H "Authorization: Bearer $HANZO_API_KEY" \
  -H 'X-Idempotency-Key: first-charge-1' \
  -H 'Content-Type: application/json' \
  -d '{
    "amountCents": 5000,
    "currency": "usd",
...
whether step 3 moves real money; step 3 answers the balance after the charge,
`processorRef` for reconciling at the processor, `status` for what the processor
did, and `test` echoing that same bucket. No `cus_…`, no `pm_…`, no `price_…` had to exist first — the org
...
| PaymentIntent, confirmed | `POST /v1/billing/topup` — amount, currency, card token, and the idempotency key as a header |
| Charge, read back | `GET /v1/billing/ledger` — the entries the charge wrote |
...
so a sandbox charge can never be read as a live one. Omitting
`X-Idempotency-Key` is survivable rather than dangerous: the server falls back to
a key derived from amount and currency, and a double-submit inside 15 minutes
