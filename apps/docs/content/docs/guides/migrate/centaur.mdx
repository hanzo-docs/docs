---
title: Centaur
description: Centaur gives a Slack thread a Kubernetes pod, a coding harness and a credential proxy. Here that is /v1/channel, /v1/agent, /v1/sandbox and /v1/integration on one origin — with a kernel boundary Centaur leaves off by default.
---
...
curl -sS -X POST https://api.hanzo.ai/v1/agent \
  -H "Authorization: Bearer $HANZO_API_KEY" \
  -H 'Content-Type: application/json' \
  -d '{"name":"eng","model":"zen5","instructions":"You are the engineering agent…",
...
curl -sS -X PUT https://api.hanzo.ai/v1/channel/agent \
  -H "Authorization: Bearer $HANZO_API_KEY" \
  -H 'Content-Type: application/json' \
...
|---|---|
| [`/v1/channel`](/docs/usage/teams) | Slack, Teams, Discord, Telegram, WhatsApp, and GitHub and Linear issues, behind one registry. Pairing and allowlist policy per org; every turn runs as the person who sent it. |
| [`/v1/agent`](/docs/concepts/agents) | Named agents with model, instructions, tools and schedule; sessions with an ordered durable event log, parent/child trees and an SSE stream. |
...
| `/v1/integration` | Org connectors and each person's own connectors; the GitHub and Linear webhooks that turn an issue comment into a channel turn, mirror issues into the todo and fire automation triggers. |
...
| Session keyed by `thread_key` | `/v1/agent/sessions` — ordered durable event log, parent/child trees, SSE stream |
...
| iron-proxy placeholder substitution | `egress` for provider keys; IAM token exchange for identity; `/v1/integration/connectors` for a person's own accounts |
| 1Password vault | Hanzo KMS — sealed secrets, per-secret agent policy, per-agent audit trail |
| Slackbot, Teamsbot, Discordbot | `/v1/channel` |
...
| Persona (`type = "persona"` tool entry) | Agent (`/v1/agent`) |
...
| Per-channel persona defaults (`SLACKBOTV2_CHANNEL_DEFAULTS`) | `PUT /v1/channel/agent` — a default agent per transport and an agent per room, set by an org admin |
...
| Centaur | Hanzo |
|---|---|
| `POST /api/session/{thread_key}` | `POST /v1/agent/sessions` |
| `POST /api/session/{thread_key}/messages` | `POST /v1/agent/sessions/{id}/events` with `kind: message` |
| `POST /api/session/{thread_key}/execute` | `POST /v1/agent/{ref}/run` naming the session |
| `POST /api/session/{thread_key}/interrupt` | `POST /v1/agent/sessions/{id}/stop` |
| `GET /api/session/{thread_key}/events?after_event_id=` | `GET /v1/agent/sessions/{id}` to replay; `GET /v1/agent/sessions/stream` to watch |
...
| `/api/slack/channels`, `/api/slack/files/…` | `POST /v1/channel/slack/send`; `/v1/integration/connectors/slack` |
...
| Console `/api/v1/secrets`, `broker_credentials`, `oauth_apps` | `/v1/kms`; `/v1/integration/connectors` |
| Console `/api/v1/principals`, `roles`, `grants`, `api_keys` | `/v1/iam`; `/v1/account/keys` |
| Console `scheduled_tasks` | `PATCH /v1/agent/{ref}` with `schedule` |
...
| `ctx.run_agent`, `ctx.run_agents([...], max_concurrency)` | `POST /v1/agent/{ref}/run`, once per agent, from the workflow |
| `ctx.post_to_slack(channel, text)` | `POST /v1/channel/slack/send` |
...
@centaur why do the billing tests fail?     @hanzo why do the billing tests fail?
  ...
  events → Postgres → SSE                     events → the session log → GET /v1/agent/sessions/stream
...
`/v1/integration/connectors` and spent only for them, so `gh pr create` from a shared channel
