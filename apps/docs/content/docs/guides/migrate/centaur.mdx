---
title: Centaur
description: Centaur gives a Slack thread a Kubernetes pod, a coding harness and a credential proxy. Here that is /v1/channel, /v1/agent, /v1/sandbox and /v1/provider on one origin — with a kernel boundary Centaur leaves off by default.
---
...
| `/v1/provider` | Org connectors and each person's own connectors; the GitHub and Linear webhooks that turn an issue comment into a channel turn, mirror issues into the todo and fire automation triggers. |
...
| iron-proxy placeholder substitution | `egress` for provider keys; IAM token exchange for identity; `/v1/connection` for a person's own accounts |
...
| `/api/slack/channels`, `/api/slack/files/…` | `POST /v1/channel/slack/send`; `/v1/connection/slack` |
...
| Console `/api/v1/secrets`, `broker_credentials`, `oauth_apps` | `/v1/kms`; `/v1/connection` |
...
`/v1/connection` and spent only for them, so `gh pr create` from a shared channel
