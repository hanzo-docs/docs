...
const { data } = await api.postChatCompletions({ frequency_penalty: 0, function_call: "<function_call>" });
```
</Tab>
<Tab value=".py">
```python
...
result = AiApi(client).post_chat_completions(frequency_penalty=0, function_call="<function_call>")
```
</Tab>
<Tab value=".go">
```go
