...
# Primary model used by Hanzo Dev. Default: "enso-auto", the Enso router's
# managed default (https://docs.hanzo.ai/docs/models/enso).
model = "enso-auto"

# Model used by the /review feature (code reviews).
review_model = "enso-auto"

# Provider id selected from [model_providers]. Default: "hanzo", which is
# https://api.hanzo.ai/v1 with the key from `dev login` or HANZO_USER_KEY.
