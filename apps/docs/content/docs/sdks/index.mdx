...
Swift, Kotlin and C++ are tagged and buildable but not on a package manager
that resolves them by name alone — Swift and C++ because neither ecosystem has
a central registry, Kotlin because its Maven Central publication has not
landed. Each page says exactly what the coordinate looks
