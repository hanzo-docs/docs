...
<Steps>
    ...
    <Step title="Accessing the Hanzo KMS API with the identity">
        ...
        Below is an example of how a Github workflow can be configured to fetch secrets from Hanzo KMS. Hanzo publishes no GitHub Action for this — the login and read are plain HTTPS calls in a `run:` step.
        ```yaml
                    ...
                    - name: Fetch secrets from Hanzo KMS
                      ...
                      run: |
                        TOKEN=$(curl -sS -X POST https://api.hanzo.ai/v1/kms/auth/login \
                          ...
                          | jq -r .accessToken)
                        VALUE=$(curl -sS "https://api.hanzo.ai/v1/kms/orgs/my-org/secrets/MY_SECRET?env=dev" \
                          -H "Authorization: Bearer $TOKEN" | jq -r .secret.value)
                        echo "::add-mask::$VALUE"
                        echo "MY_SECRET=$VALUE" >> "$GITHUB_ENV"
        ```

        Preceding steps can then use the secret values injected onto the workflow's environment.

        <Tip>
            The login call returns a short-lived access token. Mask each fetched value with `::add-mask::` before writing it to `$GITHUB_ENV` so it is redacted from the workflow logs.
        </Tip>
