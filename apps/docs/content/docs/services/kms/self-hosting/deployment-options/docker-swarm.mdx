...
The Docker stack file used in this guide defines the services and their configurations for deploying Hanzo KMS in a highly available manner. Hanzo does not publish a ready-made Swarm bundle — build the stack from the single-node [`compose.yml`](https://github.com/luxfi/kms/blob/main/compose.yml) in the KMS repository and add the HA pieces described below. The main components of this stack are as follows.
...
<Steps>
  ...
  Hanzo does not publish these three assets as a downloadable bundle — author them from the components listed above:
  - `kms-stack.yaml` — the service definitions, derived from [`compose.yml`](https://github.com/luxfi/kms/blob/main/compose.yml) plus the etcd and replica services described in the stack overview
  - `haproxy.cfg` — the load-balancing and health-check rules for PostgreSQL (5433/5434), Redis (6379) and the KMS backend (8080)
  - `.env` — the settings enumerated in [`config.yaml`](https://github.com/luxfi/kms/blob/main/config.yaml)
  </Step>
