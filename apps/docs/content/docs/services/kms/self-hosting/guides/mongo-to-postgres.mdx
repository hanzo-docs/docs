...
This guide will provide step by step instructions on migrating your Hanzo KMS instance running on MongoDB to the PostgreSQL version of Hanzo KMS.

<Warning>
  This guide applies only to the legacy Node.js KMS deployment, which is archived at [hanzoai/kms-v1](https://github.com/hanzoai/kms-v1). Current Hanzo KMS is the Go server [luxfi/kms](https://github.com/luxfi/kms) and never used MongoDB — if you are running it, no migration applies. The migration script and the `backend/` tree referenced below live only in the archived repository.
</Warning>
...
<Steps>
  <Step title="Clone the archived Hanzo KMS repository">
    The migration script ships in the archived v1 repository. Clone it, then check out the last MongoDB-capable release tag.
    ```
    git clone https://github.com/hanzoai/kms-v1.git
    ```
  </Step>
