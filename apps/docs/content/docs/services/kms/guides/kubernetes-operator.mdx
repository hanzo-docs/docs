...
The manifests for this step are given in full below. Create a working directory for them and make sure you are in it before running the `kubectl apply` commands:

```console 
mkdir -p kubernetes-operator-demo && cd kubernetes-operator-demo
```
