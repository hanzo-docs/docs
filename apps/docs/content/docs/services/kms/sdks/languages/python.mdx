...
1. **Removal of `rest`**: The SDK no longer exposes the entire Hanzo KMS API. This was nessecary as we have moved away from using an OpenAPI generator approach. We aim to add support for more API resources in the near future. If you have any specific requests, raise them through your Hanzo support channel.
