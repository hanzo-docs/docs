...
<Warning>
  The C++ SDK is not published yet — there is no repository for CMake to fetch, so the `FetchContent_Declare` below cannot resolve as written. This page documents the intended interface. Until the library ships, call the [KMS API](/docs/services/kms) directly with cURL, which is what the SDK wraps.
</Warning>
...
FetchContent_Declare(
  kms
  GIT_REPOSITORY <url-of-the-C++-SDK-once-published>
...
More examples can be found in the `examples/` folder of the SDK source tree.
