...
Hanzo does not publish a guide-assets repository, so there is nothing to clone — the task definition, agent config and Dockerfile this guide relies on are each given in full below.
Create an empty `aws-ecs-with-agent` directory and assemble them there; the guide will assume it is your working directory going forward.
