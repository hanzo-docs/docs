...
<Steps>
  <Step title="Creating the Working Directory">
    Hanzo does not publish a guide-assets repository, so there is nothing to clone. Create an empty `docker-swarm-with-agent` directory — the stack file and agent config given below go in it, and it is the working directory for the rest of this guide.
  </Step>
