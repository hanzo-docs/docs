...
| **address** | Where an operation is reached: method plus path, e.g. `POST /v1/agent`. | endpoint, route, URL, URI |
...
| **Zen** | The generative model family: open weights, for reasoning, code, vision, speech and embeddings. [Zen](/docs/models/zen). |
| **Enso** | The router: picks the model for each request (`model: "auto"`), and the `enso-*` family that serves the choice under one name. [Enso](/docs/models/enso). |
| **Kai** | The decision model: typed, calibrated answers to bounded questions, no generated text. [Kai](/docs/models/kai). |
| **Hanzo Decision** | Kai, Decision Programs, Decision Packages and policy, served at `POST /v1/decisions`. |
| **Decision Program** | A versioned graph of typed nodes — deterministic, Kai, Zen, solver, human, policy — over evidence. |
| **Decision Package** | The hashed, replayable record of one program run. |
