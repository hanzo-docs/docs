...
Three ideas make the rest of this reference cohere, and all three are shipped. Read them in this order if you are new: what is declared, how it travels, and who is calling.

<Cards>
  <Card title="The One Registry" href="/docs/architecture/registry" description="A typed op is the single thing anyone authors. REST, the OpenAPI document, the SDKs, the CLI and the MCP tools are all projected from it." />
  <Card title="Transport" href="/docs/architecture/transport" description="ZAP is the primary wire and HTTP a secondary view of the same routes. Local is a unix socket, remote is HTTPS — measured, and with an end condition." />
  <Card title="Auth" href="/docs/architecture/auth" description="What a token proves, how an org and a project scope it, why you can never send X-Org-Id, and when a 401 differs from a 403." />
</Cards>
