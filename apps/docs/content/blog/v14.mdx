---
title: Hanzo Docs v14
description: Introducing Hanzo Docs v14
author: Fuma Nama
date: 2024-09-19
---
...
Taking some inspirations from [Shadcn UI](https://ui.shadcn.com), I decided to make Hanzo Docs easier to use, with more APIs that simplify the design.
...
Clone Hanzo Docs UI components to your workspace and modify them.

```package-install
npm install @hanzo-docs/cli --save-dev
```
...
Previously, multiple page tree is implemented with Hanzo Docs UI `RootToggle` component.
You have to add it to the sidebar banner which isn't as intuitive as other APIs.

With Sidebar Tabs, by creating a root folder, Hanzo Docs will immediately add a tabs component to the sidebar.
...
In favour of this, the new search client of Hanzo Docs is introduced with support for different API clients.
...
I'm always working to reduce the dependencies required for Hanzo Docs, that is one reason why we separated Hanzo Docs into different packages.

Hanzo Docs UI now bundles icons from `lucide-react`, for Next.js apps that is not using Lucide, they can avoid downloading the entire icon library.
...
To improve code organization, we made a Metadata Image API on Hanzo Docs Core.
It is a tiny wrapper over Next.js Metadata API, composes seamlessly with Source API.

```ts title="lib/metadata.ts"
import { createMetadataImage } from '@hanzo/docs/core/server';
import { source } from '@/lib/source';
...
In favour of this, the `getImageMeta` function from `hanzo-docs-ui/og` has been removed.

### Shorthands

As you may notice, we introduced more abstractions to reduce the setup steps required for enabling some features.
It is also required for the code generator from Hanzo Docs CLI to work.
...
Hanzo Docs UI Card can now support usage without `href`.
You can pass children as React node, Typography styles will be applied correctly.

```mdx
<Card icon={<Database />} title='Content Source'>

The input/source of your content, it can be a CMS, or local data layers like [Content Collections](https://www.content-collections.dev) and [Hanzo Docs MDX](/docs/mdx).
...
Hanzo Docs v14 is compatible with Next.js 15, supporting sync and async usages of dynamic APIs.
...
We moved Twoslash UI components to `hanzo-docs-twoslash`.
This isolates some logic from Hanzo Docs UI, allowing a better code organization.
...
The types from `hanzo-docs-core/search/shared` is moved to `hanzo-docs-core/server`.
