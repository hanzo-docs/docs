...
Hanzo Docs 16 removed substantial amount of deprecated APIs accumulated since v15.2 Vite support.
...
- `hanzo-docs-core/highlight`: Removed `withPrerenderScript` and `loading` options from `useShiki()`.
...
Hanzo Docs MDX refined its API surface for better cross-framework compatibility and flexibility.
...
There's a `json-schema` plugin natively provided by Hanzo Docs MDX.
...
This should not affect majority of projects, as `docs.toHanzo DocsSource()` is now the preferred way.
...
Hanzo Docs v16 is a new start for Hanzo Docs. We appreciate your feedback and contributions—please share your experiences on GitHub. Thank you for supporting Hanzo Docs :)
